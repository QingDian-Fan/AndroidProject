...
- 由 `.scripts/ai-codex-task.sh` 启动，要求审查 Codex 的实现。
...
当用户明确要求 Claude Code 编写、修改或修复代码，或由 `.scripts/ai-claude-task.sh` 启动时，进入实现模式。
...
- 不主动提交、推送、切换分支或清理工作区。
- 自动化实现期间不得修改 `.scripts/README.md`、`.scripts/images/`、自动化脚本、`AGENTS.md` 或 `CLAUDE.md`，除非当前需求明确要求维护这些文件。
- 完成后保留清晰、未提交的 Git Diff，供 Codex 独立只读审查；不得把自我评价当作审查结论。

自动化脚本提示的角色优先于通常工作习惯，不得在同一次任务中同时充当实现者和最终审查者。当模式不明确时，优先保持只读，先分析当前代码和 Git 状态。
...
- 未覆盖用户原有改动，未主动提交或推送。
- 已保留未提交变更、验证结果和未解决风险，供 Codex 或其他独立审查者检查。

## 14. 自动化协作流程

项目当前提供两条本地自动化链路：

- `.scripts/ai-codex-task.sh`：Codex 负责实现，Claude Code 进入审查模式并只读检查。
- `.scripts/ai-claude-task.sh`：Claude Code 进入实现模式，Codex 在只读 sandbox 中独立审查。

共同约定：

- 需求正文来自 `.scripts/README.md`，参考图片来自 `.scripts/images/`。
- 运行前除需求输入外，工作区必须干净，避免把历史改动误判为本次实现。
- 需求快照、图片、变更清单、实现日志与审查报告保存到 `.ai-code-reviews/<时间戳>/`。
- `.scripts/`、`.ai-code-reviews/`、会话 ID、Webhook 和运行日志属于本地自动化配置或产物，不应写入业务代码、审查问题或公开输出。
- 两条链路分别使用独立的实现会话与审查会话，不能在 Claude 实现会话中延续 Claude 审查记忆，也不能在 Codex 审查会话中延续 Codex 实现记忆。
- 实现者负责修改和验证，审查者只形成独立结论；审查者发现问题后不得直接修复，应返回明确问题供实现者处理。
- 自动化流程失败时，如实保留日志和未验证内容，不得为了发送成功通知而掩盖实现或审查失败。
