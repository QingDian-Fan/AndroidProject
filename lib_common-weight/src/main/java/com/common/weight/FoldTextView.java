...
public class FoldTextView extends AppCompatTextView {
    ...
    public FoldTextView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        mTailColor = a.getColor(R.styleable.FoldTextView_tailTextColor, DEFAULT_TAIL_TEXT_COLOR);
        mCanFoldAgain = a.getBoolean(R.styleable.FoldTextView_canFoldAgain, DEFAULT_CAN_FOLD_AGAIN);
     ...
     * @return 展开状态下的Span
     */
    private SpannableString createUnFoldSpan(CharSequence text) {

        SpannableStringBuilder destStr = new SpannableStringBuilder(text);
        destStr.append(mFoldText);
    ...
    private CharSequence tailorText(CharSequence text) {
        ...
        if (layout.getLineCount() > getFoldLine()) {
            ...
            System.out.println((DEFAULT_ELLIPSIZE + mUnFoldText).length());
            CharSequence subText = text.subSequence(0, index - 1);
//            System.out.println(index + "" + subText);
