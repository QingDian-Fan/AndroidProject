package com.demo.project.utils.ext
...
inline fun ViewGroup.RadioButton(
    style: Int? = null,
    autoAdd: Boolean = true,
    init: AppCompatRadioButton.() -> Unit
): AppCompatRadioButton {
    val radioButton =
        if (style != null) AppCompatRadioButton(
            ContextThemeWrapper(context, style)
        ) else AppCompatRadioButton(context)
    return radioButton.apply(init).also { if (autoAdd) addView(it) }
}

inline fun ViewGroup.CheckBox(
    style: Int? = null,
    autoAdd: Boolean = true,
    init: AppCompatCheckBox.() -> Unit
): AppCompatCheckBox {
    val checkBox =
        if (style != null) AppCompatCheckBox(
            ContextThemeWrapper(context, style)
        ) else AppCompatCheckBox(context)
    return checkBox.apply(init).also { if (autoAdd) addView(it) }
}
...
inline fun ViewGroup.RadioGroup(
    style: Int? = null,
    autoAdd: Boolean = true,
    init: RadioGroup.() -> Unit
): RadioGroup {
    val radioGroup =
        if (style != null) RadioGroup(
            ContextThemeWrapper(context, style)
        ) else RadioGroup(context)
    return radioGroup.apply(init).also { if (autoAdd) addView(it) }
}
...
inline fun Fragment.RelativeLayout(
    style: Int? = null,
    init: RelativeLayout.() -> Unit
): RelativeLayout? = context?.let {
    if (style != null) RelativeLayout(
        ContextThemeWrapper(it, style)
    ) else RelativeLayout(it)
}?.apply(init)
