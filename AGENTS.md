...
本文件用于指导 Codex 在 AndroidProject 中分析、实现和验证需求。
...
## 12. 交给 Claude Code 的审查边界

Codex 完成后不要调用 Claude Code 修改代码，也不要把自我评价当作审查结论。独立审查应以用户原始需求、开始时基准提交、当前 Git 状态、完整 Diff、未跟踪文件和实际构建/测试结果为依据。
